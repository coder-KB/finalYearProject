export class DNA {
    ...
    static setTarget(data) {
        ...
        data.forEach(d => {
            if(d.isLab) {
                ...
                DNA.teacherToSubject.set(ch, d.subCode);
                DNA.subjectToTeacher.set(d.subCode, ch);
                DNA.totalSize -= 3;
                for(let i = 0; i < 3; ++i) {
    ...
    calcFitness() {
        ...
        DNA.target.forEach(sub => {
            if(sub.isLab) return;
            let requiredVal = DNA.subjectToTeacher.get(sub.subCode);
    ...
    crossover() {
        ...
        for(let i = 0; i < someChanges; ++i) {
            ...
            if(!DNA.labs.has(child.genes[a]) && !DNA.labs.has(child.genes[b])) {
        ...
        for(let day = 0; day < 6; ++day) {
            ...
            if(moveLeft) {
                let breakIndex = 0;
                while(breakIndex < breakPositions.length && end > breakPositions[breakIndex]) {
                    if(DNA.labs.has(child.genes[end])) {
                        end -= 3;
                    }
                    else if(child.genes[end] != " ") {
                        [child.genes[breakPositions[breakIndex]], child.genes[end]] = 
                        [child.genes[end], child.genes[breakPositions[breakIndex]]];
                        end--;
                        breakIndex++;    
                    }
                    else end--;
                ...
                while(breakIndex >= 0 && start < breakPositions[breakIndex]) {
                    if(DNA.labs.has(child.genes[start])) {
                        ...
                        [child.genes[breakPositions[breakIndex]], child.genes[start]] = 
                        [child.genes[start], child.genes[breakPositions[breakIndex]]];
                        start++;
                        breakIndex--;    
                    }
                    else start++;
    ...
    display() {
        ...
        this.genes.forEach(g => {
            timeTable.push([DNA.teacherToSubject.get(g), DNA.subjectToTeacher2.get(g)]);
        })
        return timeTable;
    }
}
